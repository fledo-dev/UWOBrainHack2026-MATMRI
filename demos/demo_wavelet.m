...
[imCS, resSqAll, RxAll, delVals,~,~,mseAll] = bfista(opFunc,data,W,lambda,x0,NitMax,opt_bfista);
...
subplot(2,3,4); plot(log10(abs(resSqAll(:,1)))-log10(abs(resSqAll(end,1)))); 
hold('all'); plot(log10(abs(RxAll(:,1)))-log10(abs(RxAll(end,1)))); 
plot(log10(abs(resSqAll(:,1)+RxAll(:,1)))-log10(abs(resSqAll(end,1)+RxAll(end,1)))); 
xlabel('iterations'); ylabel('log10(cost fcn values)')
legend('Data consistency', 'wavelet', 'net cost function')
subplot(2,3,5); plot(log10(delVals(:,1))); hold('all')
plot(log10(sqrt(delVals(:,2)))); 
ylabel('log10(normalized difference)'); xlabel('iterations'); legend('diff(cost)/cost','||diff(image)|| / ||image||')
